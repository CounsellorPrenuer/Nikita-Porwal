...
async function cleanAndPopulate() {
    console.log("Step 1: Deleting ALL old pricing data from Sanity...");

    // Delete all old pricingCategory documents
    const oldCats = await sanityWriteClient.fetch(`*[_type == "pricingCategory"]._id`);
    for (const id of oldCats) {
        await sanityWriteClient.delete(id);
        console.log(`  Deleted category: ${id}`);
    }

    // Delete all old pricingPackage documents
    const oldPkgs = await sanityWriteClient.fetch(`*[_type == "pricingPackage"]._id`);
    for (const id of oldPkgs) {
        await sanityWriteClient.delete(id);
        console.log(`  Deleted package: ${id}`);
    }

    // Delete all old customPackage documents
    const oldCustom = await sanityWriteClient.fetch(`*[_type == "customPackage"]._id`);
    for (const id of oldCustom) {
        await sanityWriteClient.delete(id);
        console.log(`  Deleted custom: ${id}`);
    }
